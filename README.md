...
```bash
docker run -d --name pg-store-data -e POSTGRES_USER=dbuser -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=store-data -p 5432:5432 postgres:13
```
...
docker run -d --name pg-store-data -e POSTGRES_USER=dbuser -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=image-metadata -p 5432:5432 --network rso postgres:13
...
kubectl --help
kubectl get nodes
kubectl create -f store-data-deployment.yaml 
kubectl apply -f store-data-deployment.yaml 
